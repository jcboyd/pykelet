...
where $F_{ij} = \sum_t\mathbbm{1}_{\{y_t = i\}}\mathbbm{1}_{\{y_{t-1} = j\}}$ and $F_{io} = \sum_t\mathbbm{1}_{\{y_t = i\}}\mathbbm{1}_{\{x_t = o\}}$. That is, for each dependency in the model, be it between states, or between a state and an observation, we define functions that respond to their identities at any time $t$. In a CRF, however, we may replace the indicator function for observations with any sort of function, typically binary, extracting rich features from a token. Thus, $F_{io} = \sum_t\mathbbm{1}_{\{y_t = i\}}f_{io}(\textbf{x})$. The set of functions, $\{f_{io}\}$, are the \emph{feature functions} that we define in the feature template files of Wapiti, as we will see in Section \ref{sec:wapiti}.

With some notational abuse we define \emph{feature functions}, $F_{k}(y_t, y_{t-1}, x_t) = \mathbbm{1}_{\{y_t = i\}}\mathbbm{1}_{\{y_{t-1} = j\}}$ for a transition feature, and $F_{k}(y_t, y_{t-1}, x_t) = \mathbbm{1}_{\{y_t = i\}}\mathbbm{1}_{\{x_{i} = o\}}$ for an emission feature. Thus, we acquire a more compact expression,
...
Now we may define the discriminative counterpart to this model to this joint distribution; the linear-chain CRF,
...
where $Z(\mathbf{x}) = \sum_{y'}\exp \Big\{\sum_k{\lambda_{ij}F_{k}(y'_t, y'_{t-1}, x_t)}\Big\}$ is known as the partition function ensuring probabilities summing to 1. Whereas Hidden Markov models effectively model only the \emph{occurence} of a word, with conditional random fields we may define arbitrarily complex features describing rich information about a word, its attributes, and its context.  From these defintions, we may define a cost function in the following way,
