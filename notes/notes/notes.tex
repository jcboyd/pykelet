...
\subsubsection{Obtaining Training Data}

\subsubsection{Improving Grobid}

Grobid . One approach to improving the tool is therefore to identify the weakest parts of the cascade . Regarding the HEP use case, it is unlikely we will find anything particular to improve; after all, a HEP date is a CORA date. This same point applies to the other lower-tier models. By the same token . In practical use, the most important models are those at the top, as without accurate classification from these, the lower levels . Segmentation provides the most interesting .  . And  although the header model is the best trained of all models (with a current training set of 2775 articles) . As we discovered, the interplay between the segmentation and header models provided an interesting starting point for our improvements.

The header model requires not only improvement through training , but also extension. In particular HEP papers require the modelling of additional fields, such as
