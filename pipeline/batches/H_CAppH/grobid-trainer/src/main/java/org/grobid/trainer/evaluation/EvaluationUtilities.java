	...
	public static String evaluateStandard(String path, Function<List<String>, String> taggerFunction) {
		...
		try {
			...
			while (stt.hasMoreTokens()) {
				...
				int ind = labels.indexOf(previousToken); // index of expected
				if (ind != -1) { // i.e. already seen expected
					if (previousToken.equals(currentToken)) {
						Integer count = counterObserved.get(ind);
						counterObserved.set(ind, count + 1);						
					} else {
						int ind2 = labels.indexOf(currentToken); // index of prediction

						if (ind2 != -1) { // i.e. already seen predicted
				...
				} else { // i.e. expected token does not yet exist in labels collection
				...
				confusionMatrix.increment(previousToken, currentToken);
			}
			...
			*/
			...
			report.append("\n===== Confusion matrix =====\n\n");
			report.append(confusionMatrix.toString()).append("\n\n");
