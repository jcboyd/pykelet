...
//		crfppTrainer = new CRFPPTrainer();
    ...
    public static void runEvaluation(final Trainer trainer) {
		...
		try {
			...
			if(GrobidProperties.isOutputToLog()) {
				log = new FileOutputStream(GrobidProperties.getLogPath() + "/" + new Timestamp(System.currentTimeMillis()).toString().replace(" ", ""));
				log.write(report.getBytes());
				log.close();
			}
		} catch (Exception e) {
